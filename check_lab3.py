...
def check_myprofile():
    ...
    code += "\nprint 'SUCCESS'"
    ret = check_profile_internal("Exercise 4: my-profile functional", code, "SUCCESS")
    if not ret:
        log(red("FAIL"), "Exercise 4: Profile my-profile doesn't seem to parse correctly")
    else:
        log(green("PASS"), "Exercise 4: basic sanity check")
...
def check_fs():
    ...
    if not file_write_check("Exercise 2: /testfile write:", tmpfile, data,
                            "test1", "supersecretpassword", "test2", "pass"):
        log(red("FAIL"), "Exercise 2: /testfile check (could not write to /testfile)")
        return

    # try to read the same file
    if file_read_check("Exercise 2: shared /testfile:", tmpfile, None, data,
                       "test2", "pass", "test1", "supersecretpassword"):
        log(red("FAIL"), "Exercise 2: /testfile check (/testfile shared by more than one user)")
        return
    ...
    if file_read_check("Exercise 2: shared /testfile:", tmpfile, None, data,
        ...
        log(red("FAIL"), "Exercise 2: /testfile check (special characters in usernames)")
        return
    ...
    log(green("PASS"), "Exercise 2: /testfile check")
...
def check_profileapi_uid():
    ...
    if l > 2:
        log(red("FAIL"), "Exercise 3: profile-service forks more than one root process")
        return
    if l < 2:
        log(red("FAIL?"), "Exercise 3: profile-service does not seem to fork")
        return

    log(green("PASS"), "Exercise 3: ProfileAPIServer uid")
...
def check_profile_service():
    ...
    if column_in_table(persondb, "person", "profile"):
        log(red("FAIL"), "Challenge:", "person table still has column profile")
        return False
    return True
