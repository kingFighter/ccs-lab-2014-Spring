...
def check_ex5():
    if not os.path.exists("/jail/echosvc/sock"):
        log(red("FAIL"), "Exercise 5:", "/jail/echosvc/sock missing before exploit")
        return
    sh("python echo-exploit.py localhost 8080 >/tmp/ex5.log 2>/dev/null", exit_onerr=False)
    if os.path.exists("/jail/echosvc/sock"):
        log(red("FAIL"), "Exercise 5:", "/jail/echosvc/sock still exists")
    else:
        log(green("PASS"), "Exercise 5")

def check_ex6():
    sh("./check-ex6.sh", exit_onerr=False)
...
def check_ex7_1():
    ...
    if access(db, S_IRWXG) or access(db, S_IRWXO):
        log(red("FAIL"), "Exercise 7:", "cred table directory has group or world permissions")
        return False
    return True
...
def check_ex7_2():
    ...
    if any([ column_in_table(persondb, "person", c) \
        ...
        log(red("FAIL"), "Exercise 7:", "person table still has some cred table column")
        return False
    return True

def check_ex7():
    check_0 = check_db("Exercise 7:", "auth-server.py",
...
def check_ex8():
    ...
    if not os.path.exists(dbfile):
        log(red("FAIL"), "Exercise 8:", "no db %s" % dbfile)
        return

    db = file_read(dbfile)
    if "supersecretpassword" in db:
        log(red("FAIL"), "Exercise 8:", "plain-text password in database")
    else:
        log(green("PASS"), "Exercise 8")

def check_ex9_1():
    if column_in_table("/jail/zoobar/db/person/person.db", "person", "zoobars"):
        log(red("FAIL"), "Exercise 9:", "person table still has the zoobars column")
        return False
    return True

def check_ex9():
    check_0 = check_db("Exercise 9:", "bank-server.py",
                       "/jail/zoobar/db/bank/bank.db",
                       "bank", ['zoobars'])

    if check_0 and check_ex9_1():
